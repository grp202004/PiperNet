...
export default class GraphStore {
    ...
    removeNodes(nodeids) {
        runInAction("remove nodes by ids", () => {
            ...
            this.rawGraph.edges = this.rawGraph.edges.filter(
                (e) =>
                    !nodeids.includes(e.source_id) &&
                    !nodeids.includes(e.target_id)
            );
        });
    }
    ...
    setUpFrame() {
        ...
        graphFrame.ee.on("right-click", (data) => {
                        ...
                        MenuItemFactory({
                            onClick: () => {
                                if (this.frame.rightClickedNode) {
                                    ...
                                    const neighbors = this.getNeighborNodesFromRawGraph(
                                        rightClickedNodeId
                                    );
                                    neighbors.sort((n1, n2) => {
                                        if (n1["pagerank"] && n2["pagerank"]) {
                                            return (
                                                n2["pagerank"] - n1["pagerank"]
                                            );
                                        }
                                        return 0;
                                    });
            ...
            ContextMenu.show(
                menu,
                { left: data.pageX, top: data.pageY },
                () => {
                    // onMenuClose
